...
The idea of a rejecting ML predictions using a treshold is that for some treshold value $\tau$ in the range $[0, 1]$, we accept all predictions with confidence values greater than or equal to $\tau$ and reject all predictions with confidence values below $\tau$.
%
...
%
For the mild conditions, we assume that $V_{tp}$ and $V_{tn}$ are expressed as gains and, therefore, positive values and $V_{fp}$, $V_{fn}$, and $V_{r}$ as costs and, therefore, negative values.
%
...
%
Conditions \ref{for:conditions-fp-fn} are satisfied under condition \ref{for:value-condition} and since we assume that $V_{fp}$, $V_{fn}$, and $V_r$ are negative.
% 
...
\end{align}
%
We can now use \ref{for:final-V} to calculate the total value of some ML model for all thresholds $\tau \in [0, 1]$.
%
...
In this section, we will explain how we apply the value-sensitive metric from section \ref{sec:value-metric} to some of the state-of-the-art automatic hate speech detection models to convert them into dependent rejectors.
%
...
%
We select the second model, an DL model, based on the findings from \citet{agrawal2018deep, badjatiya2017deep}.
%
...
%
We use the DistilBERT model since it's faster to train and smaller in size compared to BERT models while achieving similar performance \citep{sanh2019distilbert}.
%
...
%
We split the dataset into a train and test dataset according to an 80:20 ratio, respectively.
%
...
%
With KDE we estimate the PDF by weighing each measured confidence value from a set of predictions using a kernel function, a gaussian density function in our case since it's the most commonly used, for each possible confidence value in the range $[0, 1]$.
%
...
%
We accomplish this by comparing the $V(\tau_O)$ values and the accuracies between all models.
