...
%
We used the optimal threshold to determine when we could trust machine predictions ($\text{confidence} \geq \tau_O$) and when we needed to pass machine predictions to a human moderator ($\text{confidence} < \tau_O$).
