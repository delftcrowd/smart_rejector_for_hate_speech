...
Most studies use supervised learning techniques that range from traditional ML to deep learning (DL) classification models, and a few use unsupervised learning techniques to cluster the social media posts.
%
...
%
Therefore, value assessment using objective measurements is already difficult for specific cases let alone in general.
%
...
%
They conclude their work by saying that there are too many problems to empirically evaluate the costs and benefits in the hate speech detection context \citep{sunstein2018does}.
%
...
%
We do only consider the social media users as the stakeholder in the subjective assessment since they are the most affected by the consequences of hate speech detection.
%
...
%
Therefore, in section \ref{sec:me}, we will explain why the technique called Magnitude Estimation seems promising for our use-case.
...
%
Likert scales are a series of multiple Likert-type questions (items) where subjects can answer questions with several response alternatives \citep{boone2012analyzing}.
%
...
%
For example, one may give rating using values of 1, 2, and 10, while another may use 100, 200, and 1000.
%
...
%
However, as opposed to the unipolar scales (with only positive values) used in \citep{bard1996magnitude, mcgee2004master, maddalena2017crowdsourcing}, we cannot apply geometric averaging to bipolar scales (disagree-agree).
%
...
%
Psychophysicists compare the magnitude estimates to the physical stimuli by analyzing the correlation between them \citep{bard1996magnitude}.
%
...
%
They found that for the same precision values the perceived value changes dependent on the user characteristics and the type of classification errors (an offensive tweet labelled as hate (low impact) and a neutral tweet labelled as hate (high impact)) \citep{olteanu2017limits}.
%
