...
As explained in section \ref{sec:reliability}, we measured the interrater reliability of the participants using Krippendorff's alpha.
%
...
We followed the approach from section \ref{sec:analysis-demographic} to analyze whether there exist statistically significant differences between groups with different demographic characteristics.
%
We focused on six different features: sex, student, continent, nationality, language, and ethnicity.
%
We decided to use non-parametric statistical tests only for analyzing demographic differences for two reasons.
%
First, we found that the assumptions of homogeneity of variances and normality were mostly violated in our dataset.
%
Second, we found that for most features, except sex, the sample sizes of the feature groups were not equal.
%
We used Mann-Whitney U to verify if there is a significant difference between two groups and we used Kruskal-Wallis for more than two groups.
%
Table \ref{tab:results-differences-ind} shows the resulting p values for all scenarios and all features.
%
We found that there are no differences between men and woman.
%
For both the student and continent features, we found that there are only three scenarios where there differences between the groups.
%
Tha nationality and the language features contain the most scenarios where there is a significant difference between the groups.
%
We observed that there exist 5 scenarios with non-hateful posts and 10 scenarios with hateful posts where there is at least one feature with significant differences between the groups.
%
Scenarios FP4 and REJ4 have the most features (4) with significant differences between the groups.
%
Scenarios TP6, FN5, and REJ1 have the second most feaures (2) with significant differences between the groups.
%
...
%
We aggregated the scores per question type by calculating the mean response value to all scenarios of the same type, e.g. TP, for each participant.
%
Then we applied the Mann Whitney U test to aggregated scores to find out whether there is a significant difference between the groups of some feature for that scenario type.
%
We found the most significant differences between different demographic groups for the aggregated scores of the FP scenarios.
%
Finally, we conducted pairwise Mann-Whitney U tests to check if there are any differences between pairs of groups for the multi-group features: nationality, langauge, and ethnicity.
%
Tables \ref{tab:results-pairwise-nationality}, \ref{tab:results-pairwise-language}, and \ref{tab:results-pairwise-ethnicity} present the resulting p values of the pairwise Mann-Whitney U tests for the features nationality, language, and ethnicity, respectively.
%
We observed that for most scenarios, we do not find any significant statistical differences between the pairs of groups for all three features.
%
We found the most differences, four out of the six pairs, for scenario FN5 when looking at the nationality feature.
%
\begin{table}
    \small
    \centering
