...
\begin{table*}[t]
    ...
    \begin{tabular}{lcccccccccc}
        ...
        \textbf{Unseen data}                     &                                 &                                         &                                  &                     &              &             &                     &              &             \\
        \midrule
        \textbf{Seen data $(V_{TP}=V_{TN}=0)$}   &                                 &                                         &                                  &                     &              &             &                     &              &             \\
        \textbf{Unseen data $(V_{TP}=V_{TN}=0)$} &                                 &                                         &                                  &                     &              &             &                     &              &             \\
        \bottomrule
    \end{tabular}
    \caption{The optimal rejection thresholds ($\tau$), accuracy of the accepted predictions (Acc), and rejection rates (RR) of all models for both datasets using the values from the survey.}
    \label{tab:metric}
\end{table*}
