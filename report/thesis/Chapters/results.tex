...
%
Positive $V(\tau)$ indicate that the model for rejection threshold $\tau$ is valuable, while negative values indicate that the costs of incorrect accepted and rejected predictions exceed the gains of correct accepted and rejected predictions.
%
...
%
The last two rows (where $V_{tp}=0$ and $V_{tn}=0$) show that we achieved higher accuracies of accepted predictions for increased optimal rejection thresholds.
%
...
%
For the \emph{unseen} data, it rejects the least amount of predictions ($92.3\%$) and achieved the highest accuracy of accepted predictions ($88.1\%$).
%
The CNN model performs the worst for all configurations in terms of accuracy of accepted predictions, and for the \emph{unseen} data the highest total value is achieved when all predictions are rejected.
...
%
It presents the total value of all models for the optimal rejection thresholds ($V(\tau_O)$), the total value when all predictions are accepted ($V(0)$), and the accuracies of all models when all predictions are accepted.
%
First, we compared the accuracy of the original model with $V(0)$ as in both cases all predictions are accepted.
%
In the first two rows, both the accuracy and $V(0)$ indicate that the DistilBERT model performs the best for both the \emph{seen} and \emph{unseen} datasets.
%
In the last two rows (where $V_{tp}=0$ and $V_{tn}=0$), both metrics indicate that the DistilBERT model performs the best for the \emph{seen} dataset, while the metrics give different results for the \emph{unseen} dataset.
%
The DistilBERT model performed the best when we use accuracy as the metric, while the CNN model performed the best when we use $V(0)$ as the metric.
%
All $V(0)$ values in the last row show that all models are not valuable for \emph{unseen} data when we accept all predictions.
%
...
%
The DistilBERT model achieved the highest $V(\tau_O)$ values in all configurations except for \emph{unseen} data with $V_{tp}=0$ and $V_{tn}=0$ as the LR model achieved a higher total value.
%
