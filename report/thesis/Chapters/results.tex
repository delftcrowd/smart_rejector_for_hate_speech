...
%
Section \ref{sec:results-value-ratios} presents the value ratios that are required for our value-sensitive rejector, section \ref{sec:results-reliability} presents the reliability analysis, and section \ref{sec:results-validity} the validity analysis.
%
...
We collected the responses of all participants to all scenarios for both surveys: one where participants use the ME scale and the other where they use the 100-level scale to judge to scenarios.
%
Figure \ref{fig:boxplots} shows the reponse values to all scenarios for both scales.
%
In general, participants agreed with the TP and TN scenarios and disagreed with the FP, FN, and rejection scenarios.
%
...
%
Scenario FN4 was an exception, as there were some particpants that agreed with the scenario for both scales.
\begin{figure}
    \centering
...
%
We also observed that participants provide greater absolute response values to the TP and TN scenarios than to the FP and FN scenarios, respectively.
%
\begin{table}
    \small
    \centering
...
%
We calculated the median because when we look at figure \ref{fig:boxplots}, we can see that the data of both scales is skewed and contains many extreme outliers.
% 
...
%
First, we found that the assumptions of homogeneity of variances and normality were mostly violated in our dataset when looking at the different groups for all features.
%
...
%
We found that there are no significant differences between men and woman.
%
For both the student and continent features, we found that there are only three scenarios where there significant differences between the groups.
%
We found the most significant differences in the nationality and language features.
%
...
%
Then we applied the Mann-Whitney U test to the aggregated scores to find out whether there is a significant difference between the groups of some feature for that scenario type.
%
We found the most significant differences for the aggregated scores of the FP scenarios.
%
Finally, we conducted pairwise Mann-Whitney U tests to check if there are any significant differences between pairs of groups for the multi-group features: nationality, langauge, and ethnicity.
%
...
%
We observed that for most of these scenarios and for all three features, we do not find many pairwise significant differences.
%
Scenario FN5 contains the most pairwise significant differences (four out of the six pairs) when looking at the nationality feature.
%
\begin{table}
    \small
    \centering
