...
\subsubsection{Data}
% TODO: explain content analysis
We will use 40 different social media posts where 20 are hateful and 20 non-hateful to create 40 different scenarios. This implies that each subject will need to judge four scenarios for each type (TP, TN, FP, FN, and rejection) for each scale type (ME and 100-level) since we let each subject use both scales. We used the datasets from \cite{waseem2016hateful} and \cite{basile2019semeval} and randomly sampled both neutral and hateful samples. Both datasets contain tweet messages from Twitter that are neutral, racist or sexist. We filtered out the Twitter replies and mentions since the context is not always clear in these messages.

\section{Procedure}
This section will explain all steps of the survey. \autoref{sec:appendix} contains all presentation texts, the informed consent, and some examples of scenarios. All subjects have 10 seconds before they can continue to the next question. There is no limit to the amount of time they spend on each question.
