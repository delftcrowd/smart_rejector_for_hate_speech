...
\subsection{Design}
We will conduct two types of surveys that are very similar but only differ in the presentation of the scenarios. Further, we will use two scales for our response scale. We will conduct the experiments using the ME scale and another bounded scale that consists of 100 numerical levels. All the collected data is completely anonymous. We will inform the participants not to put personal identifiers in their answers.

\subsubsection{Experiment Type 1: individual scenarios}
In this experiment, we will shuffle and present the individual TP, TN, FP, FN, and rejection scenarios to the subjects.

\subsubsection{Experiment Type 2: pairs of scenarios}
In this experiment, we will shuffle and present the TP, TN, FP, FN, and rejection scenarios in pairs to the subjects. This means that we will end up with $n(n-1)/2 = 5(5-1)/2 = 10$ different pairs.

\subsubsection{Scales: ME and 100-level scales}
We will use two scales in our experiments: the ME scale and the 100-level scale. The 100-level scale is a bounded scale that consists of 100 numerical levels. We will use the 100-level scale to validate whether we can use the ME scale to measure the subjects' opinions about the different types of scenarios. This validation is a form of convergent validation where we try two types of measures to see if they measure the same thing \cite{krippendorff2004reliability}. The 100-level scale is easier to understand than ME, does not require normalization, and provides more flexibility than Likert scales \cite{maddalena2017crowdsourcing}.
