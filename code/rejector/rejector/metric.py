...
class Metric:
    ...
    def calculate_effectiveness(self, threshold: float, use_pdf: bool = True) -> float:
        ...
        if use_pdf:
            ...
            return (
                (value_TP + value_rejection) * tps.integral(min=threshold, max=1.0)
            ...
            return (
                (value_TP + value_rejection)
                * Prediction.count_above_threshold(tps, threshold)
                + (value_TN + value_rejection)
                * Prediction.count_above_threshold(tns, threshold)
                + (value_rejection - value_FP)
                * Prediction.count_above_threshold(fps, threshold)
                + (value_rejection - value_FN)
                * Prediction.count_above_threshold(fns, threshold)
                - (value_rejection + value_TP)
                * Prediction.count_below_threshold(tps, threshold)
                - (value_rejection + value_TN)
                * Prediction.count_below_threshold(tns, threshold)
                + (value_FP - value_rejection)
                * Prediction.count_below_threshold(fps, threshold)
                + (value_FN - value_rejection)
                * Prediction.count_below_threshold(fns, threshold)
            )
    ...
    def plot_pdfs(self) -> None:
        ...
        plot_conf = [
            {"plt_y": 0, "plt_x": 0, "data": self.pdfs.tps, "title": "True Positives"},
            {"plt_y": 0, "plt_x": 1, "data": self.pdfs.tns, "title": "True Negatives"},
            {"plt_y": 1, "plt_x": 0, "data": self.pdfs.fps, "title": "False Positives"},
        ...
        for conf in plot_conf:
            ...
            axs[conf["plt_y"], conf["plt_x"]].set_xlabel("Reliability value")
    ...
    def plot_effectiveness(self, use_pdf: bool = False) -> None:
        ...
        pyplot.plot(
            thresholds[index], max_effectiveness, marker="o", markersize=3, color="red"
        )
        pyplot.annotate(
    ...
    def print_optimal_threshold_stats(self) -> None:
        ...
        accepted = list(
            filter(lambda p: p.predicted_value >= optimal_threshold, self.predictions)
        )
    ...
    def plot_multiple_effectiveness(
        ...
        for index, (label, metric) in enumerate(metrics):
            eff = list(
                map(
                    lambda t: metric.calculate_effectiveness(t, use_pdf=use_pdf),
                    thresholds,
                )
            )
    ...
    def plot_accuracy_rejection_curves(
        ...
        for index, (label, metric) in enumerate(metrics):
            sorted_predictions = sorted(
                metric.predictions, key=lambda x: x.predicted_value
            )
            y = []
            effectiveness = list(
                map(lambda t: metric.calculate_effectiveness(t, use_pdf=use_pdf), X)
            )
            (max_index, max_eff) = cls.maximum_effectiveness(effectiveness)
            optimal_threshold = X[max_index]
            rejected = list(
                filter(
                    lambda p: p.predicted_value < optimal_threshold, metric.predictions
                )
            )
            ...
            pyplot.plot(
                X, y, color=colors[index], label=f"{label}", zorder=1, linewidth=3
            )
